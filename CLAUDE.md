...
## 🚦 Current Version: v0.4.19 (🟡 เขียนเสร็จ + ทดสอบผ่าน preview แล้ว — ยังไม่ push — ⚠️ รอรัน 3 SQL: `scripts/sql_promo_scope.sql`, `scripts/sql_packages_rls.sql`, `scripts/sql_promo_stack.sql`)
...
*Last updated: 5 ก.ค. 2026 — v0.4.19 🟡 เขียนเสร็จ + ทดสอบผ่าน preview ครบ ยังไม่ push — **เพิ่ม auto-scroll การ์ด Collage หน้าแรกให้ลูกค้าจริงเห็นด้วย** (เดิมมีแค่ใน HE preview ไม่เคยต่อมาที่ `liff_customer.html` เลย ดูหัวข้อ v0.4.19 ในตารางด้านบน) — v0.4.12–v0.4.18 (รายละเอียดในตารางด้านบน): ระบบ Promo Scope + ซ้อนโค้ด/scope ยกเว้น (⚠️ รอรัน `scripts/sql_promo_scope.sql` + `scripts/sql_promo_stack.sql`), แก้บั๊กแพคเกจสร้างไม่ได้ (⚠️ รอรัน `scripts/sql_packages_rls.sql`), รื้อระบบ Card HE เป็น 4 ฟอแมต + แก้บั๊กหลายจุด (scroll, ทดลองสั่ง, ลากเรียง PTT/Card, สัดส่วนเต็มผืน) — เรื่องแต้มสะสม/tier ยัง **หยุดรอนัท** (ดู Backlog) — วิสัยทัศน์พี่เก่ง/Lalamove ยังเป็น future scope — นัทกำลังเตรียมทริปแคนาดา 7 วัน (รอสัญญาณ "พร้อมไปแคนาดาแล้ว") — ถัดไป: push v0.4.19 + รอนัทรัน 3 SQL ค้าง + รอตัดสินใจเรื่องจัดวางเมนู S/D รายสัปดาห์ (เสนอ sort_order ไปแล้ว) + รอฟีดแบ็กคำโปรยมีลแพลน + รอนัทตัดสินใจเรื่อง stock_total ว่างสั่งไม่จำกัด (55/77 เมนูออนไลน์ยังไม่ตั้งสต็อก — เสนอ 3 ทางเลือกไปแล้ว)*
