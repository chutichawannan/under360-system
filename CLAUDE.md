...
## 🚦 Current Version: v0.4.21 (🟡 เขียนเสร็จ + ทดสอบผ่าน preview แล้ว — ยังไม่ push — ⚠️ รอรัน 3 SQL: `scripts/sql_promo_scope.sql`, `scripts/sql_packages_rls.sql`, `scripts/sql_promo_stack.sql`)
...
*Last updated: 5 ก.ค. 2026 — v0.4.21 🟡 เขียนเสร็จ + ทดสอบผ่าน preview ครบ ยังไม่ push — **เพิ่ม anchor point "เมนูเฉพาะ"** ให้การ์ดหน้าแรกเลื่อนตรงไปที่เมนูใดเมนูหนึ่งเป๊ะๆ (`scrollIntoView` ผูก DOM id ตรงๆ ไม่ใช่กะระยะพิกเซล) — นัทตัดสินใจทำ feature นี้จริง (เดิมแนะนำ sort_order เฉยๆ แต่นัทมีไอเดีย anchor point เอง เลยทำแบบนี้แทน ใช้กับเมนู S/D ประจำสัปดาห์ได้) — v0.4.20 ก่อนหน้า: ปรับปรุงหน้า OH "แพลนเมนู" ครั้งใหญ่: panel เมนูผูกกับวันที่ที่กำลังกรองจริงแล้ว, แก้บั๊ก checkbox ค้าง, เพิ่มสรุปรายวัน, วันที่เลยมาแล้วย้ายไปประวัติแบบพับเก็บ ⚠️ **ระวัง**: หน้านี้เขียนข้อมูลจริงลง Supabase ทันทีที่ทดสอบ (เจอระหว่างเทส revert คืนแล้ว) — v0.4.19 ก่อนหน้า: **เพิ่ม auto-scroll การ์ด Collage หน้าแรกให้ลูกค้าจริงเห็นด้วย** (เดิมมีแค่ใน HE preview) — v0.4.12–v0.4.18 (รายละเอียดในตารางด้านบน): ระบบ Promo Scope + ซ้อนโค้ด/scope ยกเว้น (⚠️ รอรัน `scripts/sql_promo_scope.sql` + `scripts/sql_promo_stack.sql`), แก้บั๊กแพคเกจสร้างไม่ได้ (⚠️ รอรัน `scripts/sql_packages_rls.sql`), รื้อระบบ Card HE เป็น 4 ฟอแมต + แก้บั๊กหลายจุด (scroll, ทดลองสั่ง, ลากเรียง PTT/Card, สัดส่วนเต็มผืน) — เรื่องแต้มสะสม/tier ยัง **หยุดรอนัท** (ดู Backlog) — วิสัยทัศน์พี่เก่ง/Lalamove ยังเป็น future scope — นัทกำลังเตรียมทริปแคนาดา 7 วัน (รอสัญญาณ "พร้อมไปแคนาดาแล้ว") — ถัดไป: push v0.4.21 + รอนัทรัน 3 SQL ค้าง + รอนัทลองใช้ dropdown "🎯 เมนูเฉพาะ" ใน HE จริงกับเมนู S/D + รอฟีดแบ็กคำโปรยมีลแพลน + รอนัทตัดสินใจเรื่อง stock_total ว่างสั่งไม่จำกัด (55/77 เมนูออนไลน์ยังไม่ตั้งสต็อก — เสนอ 3 ทางเลือกไปแล้ว)*
