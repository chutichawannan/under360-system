...
## 🚦 Current Version: v0.4.18 (🟡 เขียนเสร็จ + ทดสอบผ่าน preview แล้ว — ยังไม่ push — ⚠️ รอรัน 3 SQL: `scripts/sql_promo_scope.sql`, `scripts/sql_packages_rls.sql`, `scripts/sql_promo_stack.sql`)
...
*Last updated: 5 ก.ค. 2026 — v0.4.18 🟡 เขียนเสร็จ + ทดสอบผ่าน preview ครบ ยังไม่ push — จากฟีดแบ็กนัทรอบต่อ: ปรับ**เต็มผืน**เป็น 288×230 (5:4 แทน ~1.57:1 เดิม กว้างไปตามที่นัทว่า) + **เพิ่มลากเรียงลำดับ Card ใน HomeGrid ได้จริง** (ไอคอน "⠿" เดิมเป็นแค่ภาพหลอก ไม่เคยผูก event เลยตั้งแต่แรก) — v0.4.17 ก่อนหน้า: ปรับจตุรัส (230×230) + เต็มผืน (360×230 ตอนนั้น) ให้สูง 230px เท่าแนวตั้ง/จิ๋วคู่ทั้งหมด แถวเรียบเสมอกัน + **แก้บั๊ก liff การ์ดหน้าแรกถูกตัดขาว ไม่สุดจอ** (เกิดจาก max-width hack ที่เพิ่มไปแก้ scroll เมื่อ v0.4.14 เอง ทำให้เกิดช่องว่างขาวตันด้านขวา — เอาออกแล้ว ปล่อยเต็มจอ การ์ดใหญ่ขึ้นพอจะ overflow เองอยู่แล้ว) — v0.4.16 ก่อนหน้า: HE: **การ์ด "จิ๋ว" 2 อันติดกันซ้อนเป็นคอลัมน์เดียวอัตโนมัติ** ตามที่นัทขอ — v0.4.15: ย้ายแถบหมวดหมู่ขึ้นเหนือแถบค้นหาใน DB + **แก้บั๊กลากเรียงลำดับ PTT ใช้ไม่ได้มา 11 วัน** (ตั้งแต่ commit `fe0eae7` 24 มิ.ย. — refactor เผลอลบ `draggable="true"` ออกจาก card ตอนเพิ่มปุ่ม expand-panel, ไม่เกี่ยวกับงานวันนี้) — v0.4.14 ก่อนหน้า: **โค้ดซ้อนกันได้ + scope แบบ "ยกเว้น"** (ทับกติกา 1-โค้ด-ต่อออเดอร์เดิม ⚠️ รอรัน `scripts/sql_promo_stack.sql`) + แก้บั๊กการ์ดหน้าแรก liff ไม่โชว์ scroll + แก้บั๊ก "ทดลองสั่ง Mealplan" เปิดผิดเป็นสัปดาห์/เดือน + ยืนยันเบอร์โทรอัตโนมัติทำงานถูกอยู่แล้ว — v0.4.13: แก้บั๊กแพคเกจสร้างไม่ได้ (RLS ⚠️ รอรัน `scripts/sql_packages_rls.sql`), แก้ไขโค้ดโปรโมชั่นได้, ปุ่มลบสินค้าหมดออกจากตะกร้า, รื้อ Card HE เป็น 4 ฟอแมต — v0.4.12: โปรโมชั่นเจาะจงสินค้า/แพคเกจ/มีลแพลน ⚠️ รอรัน `scripts/sql_promo_scope.sql` ด้วย — เรื่องแต้มสะสม/tier ยัง **หยุดรอนัท** (ดู Backlog) — วิสัยทัศน์พี่เก่ง/Lalamove ยังเป็น future scope — นัทกำลังเตรียมทริปแคนาดา 7 วัน (รอสัญญาณ "พร้อมไปแคนาดาแล้ว") — ถัดไป: push v0.4.18 + รอนัทรัน 3 SQL ค้าง + รอตัดสินใจเรื่องจัดวางเมนู S/D รายสัปดาห์ (เสนอ sort_order ไปแล้ว ไม่ต้องเขียนโค้ดเพิ่ม) + รอฟีดแบ็กคำโปรยมีลแพลน + รอนัทตัดสินใจเรื่องระบบ stock_total ว่างสั่งไม่จำกัด (55/77 เมนูออนไลน์ยังไม่ตั้งสต็อก — เสนอ 3 ทางเลือกไปแล้ว)*
