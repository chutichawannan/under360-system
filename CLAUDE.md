...
5. **`HT-` (LIFF) กับ `HS-` (Hato Store สาขากรุงธนบุรี) คนละช่อง ห้ามรวมเป็นตัวเดียว**
...
### ⛔ ห้ามประกาศ "% ยอดตก" หรือ "ยอดร้านรวม" จนกว่าจะครบ 2 เงื่อนไข
**เกือบพลาดใหญ่ 27 ก.ค.:** ทั้งโปรเจคสรุปไปแล้วว่า *"ยอดตก 37–49% · ก.ค. ต่ำสุดใน 25 เดือน"* → **ฐานผิด** เพราะ **HS- ปี 2026 ขาด 6 เดือน** (ม.ค. + มี.ค.–ก.ค. — migrate ยืนยันว่า **ต้นทางออกรายงานมาเกือบว่าง ไม่ใช่ import bug** · ก.ค. ไม่มีไฟล์เลย) · ⚠️ ส.ค.–ก.ย. 2025 ก็ดูต่ำผิดปกติ อาจขาดด้วย
1. migrate ดึง **HS- ปี 2026** กลับมาครบ
2. **cross-check กับ statement ธนาคาร** ← HS- จ่าย transfer/promptpay ทั้งหมด **ถ้าเป็นเงินจริงต้องโผล่ในบัญชี = ตัวชี้ขาดเดียว**

**ที่รู้เรื่อง HS- ตอนนี้:** ทุกใบ tag `admin_notes = "Hato Store สาขา กรุงธนบุรี"` · ไม่มี `line_uid`/ชื่อ/`order_items` แต่**มี** `customer_id`+เบอร์ · ลูกค้า **90.6% เป็นคนเดียวกับ HT-** · ซ้ำกับ HT- (วันเดียวกัน+ยอดตรง) **26.5%** (control 2%) · **Hato official นับ HT- อย่างเดียว** (ต.ค. 2025: official 555/฿491,109 vs HT- 556/฿491,787)

**สิ่งเดียวที่พูดได้ตอนนี้ (ฐาน HT- ครบ 25/25 เดือน):** ช่องออนไลน์อ่อนลงในปี 2026 · **ใบ ม.ค. 336 → มิ.ย. 217 (−35%) แต่ยอด/ใบนิ่ง (~1,730→1,685)** → **ปัญหาคือ "ซื้อถี่น้อยลง" ไม่ใช่ "ซื้อน้อยลงต่อครั้ง"** = งาน retention/win-back (เอิธ) ไม่ใช่ลดราคา/อัพเซล
