...
> ✅ เช็คตรงกับ Supabase จริงแล้ว (2026-07-05 ผ่าน REST): `activity_log`, `orders.want_utensils`, `mp_offer_sets` มีอยู่แล้วทั้งหมด (นัทรันไปแล้ว) — เหลือแค่ 1 ตัวข้างล่างนี้จริงๆ
...
## 🚦 Current Version: v0.4.12 (🟡 เขียนเสร็จ + ทดสอบผ่าน preview แล้ว — ยังไม่ push — ⚠️ รอรัน 1 SQL: `scripts/sql_promo_scope.sql` — ดูหัวข้อ "🎁 Promo Scope + โค้ดแนะนำ" ด้านล่าง)
...
| v0.4.12 | โปรโมชั่นเจาะจงสินค้า/แพคเกจ/มีลแพลน + โค้ดแนะนำที่เช็คเอาท์ — ดูหัวข้อ "🎁 Promo Scope + โค้ดแนะนำ" ด้านล่าง ⚠️ รอรัน `scripts/sql_promo_scope.sql` | (ยังไม่ push) |
...
## 🎁 Promo Scope + โค้ดแนะนำ (v0.4.12 — เขียนเสร็จ ทดสอบผ่าน preview แล้ว — ⚠️ รอรัน `scripts/sql_promo_scope.sql`)

**ที่มา:** นัทอยากทำโค้ดส่วนลดที่ใช้ได้เฉพาะสินค้า/แพคเกจ/มีลแพลน (เช่น "สั่งแพคทดลองลด 100 บาท") + อยากมี toggle ว่าโค้ดไหน "แนะนำ" ให้ลูกค้าเห็นเลยที่เช็คเอาท์ (แตะใช้ได้ทันที) ส่วนโค้ดที่ไม่แนะนำ ลูกค้าต้องพิมพ์เองให้ถูก (โค้ดลับ/แจกเฉพาะกลุ่ม)

**Schema ใหม่ (`promo_codes`):**
...
**Admin (`operation_hub.html` แท็บโปรโมชั่น):** ฟอร์มสร้างโค้ดมีช่อง "ใช้ได้กับ" + picker เลือกเมนู/แพคเกจ/มีลแพลน (มีช่องค้นหาเมื่อเลือก "เมนู" เพราะมี 70+ รายการ) + checkbox "🌟 แนะนำให้ลูกค้าเห็นตอนเช็คเอาท์" — การ์ดโค้ดในลิสต์โชว์ badge scope + badge แนะนำด้วย

**Customer (`liff_customer.html` หน้าตะกร้า):** โชว์ chip "🎁 โค้ดแนะนำสำหรับคุณ" เหนือช่องกรอกโค้ด (กรองเฉพาะโค้ดที่ `show_suggested=true` + ยังไม่หมดอายุ/โควต้า/ถึงขั้นต่ำ + ตะกร้ามีสินค้าตรง scope) แตะ chip = ใช้โค้ดทันที ไม่ต้องพิมพ์

**⚠️ กติกาที่ตั้งใจออกแบบไว้ — ใช้โค้ดได้ครั้งละ 1 โค้ดต่อออเดอร์เท่านั้น (ห้ามซ้อนโค้ด):**
นัทถามไว้ว่า "ถ้ามีโค้ดหลายตัว ใช้ซ้ำกับโค้ดอื่นได้ไหม" — ตัดสินใจให้เป็น**ไม่ได้** ตั้งใจ เพราะ (1) ป้องกันบั๊ก/ช่องโหว่ส่วนลดซ้อนที่คาดเดายาก (2) ตรงกับสถาปัตยกรรมเดิมอยู่แล้ว (`appliedPromo` เป็นตัวแปรเดี่ยว ไม่ใช่ array) การแตะ chip ใหม่ หรือพิมพ์โค้ดใหม่ จะ**แทนที่**โค้ดเดิมเสมอ ไม่มีทางกดติ๊กสองโค้ดพร้อมกันได้ — ถ้าจะให้ FREESHIP ใช้ร่วมกับโค้ดลดเงินสดได้ในอนาคต ต้องออกแบบเพิ่ม (ยังไม่ทำ เพราะยังไม่มีคนขอ)

**Capability-check:** ถ้ายังไม่รัน SQL — OH ซ่อน picker (โชว์ข้อความแจ้งเตือนแทน) และไม่ส่ง 3 ฟิลด์ใหม่ตอนบันทึก โค้ดแบบ "ทั้งร้าน" เดิมยังสร้าง/ใช้งานได้ปกติทุกอย่าง — ฝั่ง LIFF ก็เช็คแบบเดียวกัน (query โค้ดแนะนำพังเงียบๆ ได้ผลลัพธ์ว่างถ้าคอลัมน์ยังไม่มี ไม่กระทบการกรอกโค้ดแบบเดิม)
...
*Last updated: 5 ก.ค. 2026 — v0.4.12 🟡 เขียนเสร็จ + ทดสอบผ่าน preview ครบแล้ว ยังไม่ push — โปรโมชั่นเจาะจงสินค้า/แพคเกจ/มีลแพลน (`promo_codes.scope_type/scope_value`) + โค้ดแนะนำที่เช็คเอาท์ (`show_suggested`, แตะ chip ใช้ได้เลย) — ตั้งกติกา **1 โค้ดต่อออเดอร์เท่านั้น ห้ามซ้อน** ตามที่นัทกังวลเรื่องบั๊ก (ดูหัวข้อ "🎁 Promo Scope + โค้ดแนะนำ") — ⚠️ รอรัน `scripts/sql_promo_scope.sql` ก่อนใช้ฟีเจอร์ scope/แนะนำ (โค้ด "ทั้งร้าน" เดิมใช้ได้ปกติแม้ยังไม่รัน) — v0.4.11 ก่อนหน้า: ระบบ "ชุด Meal Plan" (`mp_offer_sets`) แก้ราคาเองได้ที่หน้า DB + "Set 2" โปรโมชั่นแยกลิ้งได้เอง — v0.4.10: จากผลเทสแอดมินจริง 7 ข้อ (sort_order, แท็บโปรโมชั่นหาย, หน้าตั้งค่า, บั๊กแต้มลูกค้าโชว์ 0, ช้อนส้อม) — ✅ ทั้ง `sql_utensils.sql`/`sql_mp_offer_sets.sql`/`sql_activity_log.sql` รันครบแล้ว (เช็คผ่าน REST 2026-07-05) — เรื่องแต้มสะสม/tier ตัดสินใจ **หยุดรอนัท** จนกว่าจะ migrate ข้อมูลระบบเก่าเสร็จ (ดู Backlog) — วิสัยทัศน์พี่เก่ง/Lalamove บันทึกไว้เป็น future scope ยังไม่เริ่มสร้าง — นัทกำลังเตรียมทริปแคนาดา 7 วัน (รอสัญญาณ "พร้อมไปแคนาดาแล้ว" ก่อนเริ่มเตรียมไฟล์ให้) — ถัดไป: push v0.4.12 + รอนัทรัน SQL ใหม่*
