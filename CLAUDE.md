...
**🔴 ต้องสร้าง — OH "set/course builder" (นัทขอ, ยังไม่มี):** ให้แอดมิน "กำหนดเองได้เลยว่าจะขายอะไร ตั้งชื่อเซ็ต ตั้งราคาเซ็ต แล้วคลิกสินค้าใส่เข้าเซ็ตได้เลย" — รองรับทั้ง (1) คอร์สสต็อค multi-round + ลูกค้าเลือกวันส่ง (2) บันเดิลผสมราคาเหมา · เป้า **"กินลูกค้าทุกความต้องการ"** (บางคนไม่สะดวก จ/พ/ศ อยากเลือกเมนู/ไซส์เอง)
  - ⚠️ ปัจจุบันมี builder แค่ 2 แบบ: **Package S/M/L** (pick-N จากเมนูที่เลือก + base_price, ส่งรอบเดียว) · **ชุด Meal Plan `mp_offer_sets`** (HP/LC จำนวนกล่องคงที่) — **ยังไม่มี** คอร์สสต็อค multi-round/เลือกวันส่ง และบันเดิลผสมราคาเหมา

**กระทบอะไร (ต้องแก้ตามความเข้าใจใหม่ — จัดลำดับ ไม่ทำรวดเดียว):**
- **เว็บ/บล็อค:** ตอนนี้ชู Meal Plan (ทำสด) มากไป → ต้องเพิ่มคอร์สรายสัปดาห์ (สต็อค) + บันเดิลด้วย · บทความ `high-protein-meal-plan` เอา "Weekly เลือกวันส่งได้" ไปปนกับ Meal Plan (ทำสด) → ต้องแยกให้ชัด (ตอนนี้ hold `published=false` ไว้)
