...
## 🚀 Current Version: **u0.4.42 · a0.6 · f0.1 · doc v6.12**

> **u0.4.42 (3 ส.ค. · P-track) — ⏰ เวลากลาง Asia/Bangkok ทุกหน้า + sync ถาวร + การ์ดเซ็ตครบ:** ล็อกเวลาไทย **12 ไฟล์** (helper `bkkDate/bkkTime/bkkParts` + `bkkNow` ใน LIFF · นาฬิกาไทยหัวจอหน้าแอดมิน) — **บั๊กจริง:** KQ/KD query ด้วยวัน UTC = **ครัวเห็นออเดอร์ผิดวันก่อน 7 โมงเช้า** · LIFF cutoff 18:00/08:30 + วัน จ/พ/ศ อิงเวลาเครื่อง (เทสจาก PST ได้ "อาทิตย์" ขณะไทย "จันทร์") · checkout **เสนอวันส่งผิด** · DB `mpIsoDate` (**ปิด Known Issue #8**) · `scripts/hato_sync.mjs` = collector ถาวร (**นัทสั่งด้วยคำว่า "sync"**) · sync 1-3 ส.ค. **+31 ใบ ฿36,260** · **การ์ดหน้าโฮมครบ 10 ใบทุกเซ็ต + รูปแบรนด์จริง** · เทสสั่งเซ็ต end-to-end ผ่าน · commit `b82d3cd` บน `feature/mcp-ploy` **รอเทส Preview → merge**
> 🔴 **สปรินต์ปัจจุบัน (นัทสั่ง 3 ส.ค.): ทุกห้องรุมที่ LIFF ให้จบ 7 ส.ค.** — P = เทสหลัก (เปิดหน้าจริงแบบมือถือ แคปชี้จุดเอ๊ะ) · u = เขียนโค้ดทั้งหมด · CC = หัวหน้าคุมคิว — ✅ **u-track cutover-prep ขึ้น main (31 ก.ค.): identity/CRM + register no-OTP + KQ/แมส fix + Lalamove scaffold** (9 งาน · ดู changelog ท้ายไฟล์) · 🔄 P-track sync (working บน branch)
