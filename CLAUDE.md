...
> ✅ เช็คตรงกับ Supabase จริงแล้ว (2026-07-05 ผ่าน REST): `activity_log`, `orders.want_utensils`, `mp_offer_sets` มีอยู่แล้วทั้งหมด (นัทรันไปแล้ว) — เหลือ 2 ตัวข้างล่างนี้
...
## 🚦 Current Version: v0.4.13 (🟡 เขียนเสร็จ + ทดสอบผ่าน preview แล้ว — ยังไม่ push — ⚠️ รอรัน 2 SQL: `scripts/sql_promo_scope.sql`, `scripts/sql_packages_rls.sql`)
...
*Last updated: 5 ก.ค. 2026 — v0.4.13 🟡 เขียนเสร็จ + ทดสอบผ่าน preview ครบ ยังไม่ push — จากฟีดแบ็กนัทรอบใหม่: แก้บั๊ก "สร้างแพคเกจไม่สำเร็จ" (RLS policy ไม่เคยมีให้ anon เขียน packages/package_items ⚠️ รอรัน `scripts/sql_packages_rls.sql`) + แก้ไขโค้ดโปรโมชั่นได้แล้วถ้ายังไม่มีคนใช้ + ปุ่มลบสินค้าหมดสต็อกออกจากตะกร้าที่หน้าเมนู + **รื้อระบบขนาด Card ใน HE ใหม่ทั้งหมด** (2 แบบ→4 ฟอแมตอิสระ จตุรัส/แนวตั้ง/เต็มผืน/จิ๋ว แก้บั๊กการ์ดใหม่เรนเดอร์ผิดขนาดจากระบบจับกลุ่ม solo เดิม, px ตรงกันเป๊ะระหว่าง HE กับลูกค้าจริงแล้ว) — v0.4.12 ก่อนหน้า: โปรโมชั่นเจาะจงสินค้า/แพคเกจ/มีลแพลน + โค้ดแนะนำที่เช็คเอาท์ (1 โค้ดต่อออเดอร์เท่านั้น ห้ามซ้อน) ⚠️ รอรัน `scripts/sql_promo_scope.sql` ด้วย — v0.4.11: ระบบ "ชุด Meal Plan" แก้ราคาเองได้ที่หน้า DB — เรื่องแต้มสะสม/tier ยัง **หยุดรอนัท** (ดู Backlog) — วิสัยทัศน์พี่เก่ง/Lalamove ยังเป็น future scope — นัทกำลังเตรียมทริปแคนาดา 7 วัน (รอสัญญาณ "พร้อมไปแคนาดาแล้ว") — ถัดไป: push v0.4.13 + รอนัทรัน 2 SQL ใหม่ + รอฟีดแบ็กคำโปรยมีลแพลนที่เสนอไป*
