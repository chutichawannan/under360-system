...
## 🔗 ลิงก์สั้นสำหรับ QR ที่พิมพ์แล้วแก้ไม่ได้ (เพิ่ม 15 ก.ย. 2569)
| ลิงก์สั้น | ชี้ไปที่ | รหัส |
|---|---|---|
| `/j` | `/prayer` (หน้าสักการะ · QR บนสติกเกอร์กล่องเจ) | **307** |

🔴 **ห้ามเปลี่ยน `/j` เป็น 301 เด็ดขาด**
301 = เบราว์เซอร์จำปลายทางไว้ถาวร · QR พิมพ์ลงสติกเกอร์แล้วแก้ไม่ได้ →
วันไหนย้ายหน้าปลายทาง เครื่องที่เคยสแกนจะวิ่งไปที่เก่าตลอดไป = สติกเกอร์ทั้งล็อตพัง
ลิงก์สั้นทุกตัวที่ไปอยู่บนของพิมพ์ (สติกเกอร์ · กล่อง · ใบปลิว) ให้ใช้ 307 เหมือนกันหมด
⚠️ `vercel.json` ใส่คอมเมนต์ไม่ได้ (Vercel ตรวจ schema เข้ม คีย์แปลกปลอม = build ไม่ผ่าน) → เหตุผลจึงอยู่ในไฟล์นี้
