...
// ═══ 🛑 สวิตช์ปิดถาวร — ตั้งเมื่อ 8 ส.ค. 2026 (วันสลับมาใช้ระบบร้านเราเอง) ═══
// ทำไมต้องปิด: สคริปต์นี้ยึด "Hato มีอะไร" เป็นความจริง แล้วปิดเมนูที่ Hato ไม่มี
//   → พอเลิกใช้ Hato มันจะเห็นว่า "Hato ไม่มีอะไรเลย" แล้วไล่ปิดเมนูทั้งร้าน = ลูกค้าสั่งอะไรไม่ได้เลย
// ตั้งแต่ตอนนี้ **หน้า DB คือที่เดียวที่ตัดสินว่าเมนูไหนเปิด/ปิด** ไม่มีสคริปต์ไหนไปเปลี่ยนอัตโนมัติอีก
// ถ้าจำเป็นต้องใช้จริง ๆ (เช่นย้อนดูเทียบข้อมูลเก่า) ให้รันแบบดูอย่างเดียว: HATO_SYNC=readonly
const KILL = process.env.HATO_SYNC;
if (KILL !== 'readonly' && KILL !== 'iknowwhatimdoing') {
  console.error('🛑 สคริปต์นี้ถูกปิดถาวรแล้ว (8 ส.ค. 2026 — เลิกใช้ Hato)');
  console.error('   เหตุผล: Hato ไม่มีข้อมูลแล้ว → ถ้ารันจะไล่ปิดเมนูทั้งร้าน ลูกค้าสั่งไม่ได้');
  console.error('   ต่อจากนี้ เปิด/ปิดเมนู ทำที่หน้า DB เท่านั้น');
  console.error('   อยากดูเฉย ๆ ไม่แก้อะไร:  HATO_SYNC=readonly node scripts/hato_menu_sync.mjs');
  process.exit(1);
}
if (KILL === 'readonly' && APPLY) {
  console.error('🛑 โหมด readonly ใช้ --apply ไม่ได้');
  process.exit(1);
}

const raw = await pull();

// 🚨 กันเคส Hato ตายแล้วแต่ยังตอบ 200 พร้อมลิสต์ว่าง/สั้นผิดปกติ
//    (แพทเทิร์นประจำบ้าน: "เครื่องมือคืนผลลวงโดยไม่มีอะไรแดง")
if (raw.length < 80) {
  console.error(`🛑 หยุด — Hato คืนมาแค่ ${raw.length} รายการ (ปกติ ~125)`);
  console.error('   แปลว่าต้นทางพังหรือถูกปิดแล้ว · ถ้าเดินต่อจะปิดเมนูทิ้งเป็นจำนวนมาก');
  process.exit(1);
}
