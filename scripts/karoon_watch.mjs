// เฝ้ากล่องจดหมายของห้องการุน (พลอยคุยผ่านบอท @613yqzdf) · แยกจากกะปัน 26 ก.ย.
...
const ROOM='karoon';                                       // ต้องตรงกับ HOME_ROOM ใน api/ploy-bot.mjs
const SENDER=encodeURIComponent('พลอย (สั่งผ่านไลน์)');
const F='karoon_inbox_last.txt';
...
for(;;){
  try{
    ...
    if(Array.isArray(md)&&md.length&&EVERY[md[0].text]&&md[0].text!==mode){
      mode=md[0].text; console.log('⚙️ การุนเปลี่ยนเป็นโหมด '+(LABEL[mode]||mode)+' — เช็คทุก '+(EVERY[mode]/1000)+' วิ');
    }
    if(Array.isArray(inb)&&inb.length){
      for(const m of inb){ console.log('📲 พลอยทักการุน: ' + String(m.text).replace(/\s+/g,' ')); last=m.created_at; }
