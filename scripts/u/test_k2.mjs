...
/* 22 ก.ย. 2569: คำเตือนเดิม 'เกลี่ยข้ามวันไม่ได้' หมดอายุแล้ว — หน้าวางแผนบอกยอดจ่อรายวันข้างหน้าได้จริงแล้ว
   เจตนาที่ต้องคงไว้: ครัวต้องถูกชี้ให้ดู 'เหลือขายได้' ไม่ใช่ 'ในตู้' (เคสจริง 21 ก.ย. ครัวดูของในตู้แล้วไม่ผลิต เช้าต่อมาของหมด) */
ok('⑥ คนพิมพ์ทับแผนได้ + หน้าชี้ให้ดู "เหลือขายได้" ไม่ใช่ "ในตู้"',
  NEW.indexOf('function savePlan(') >= 0 && NEW.indexOf('ในตู้</b> รวมของที่ขายไปแล้วรอจัด') >= 0);
ok('⑥b ทุกเมนูบอกเหลือขายได้จริง + จ่อข้างหน้ารายวัน + เตือนวันที่จะขาด',
  NEW.indexOf('function leftLine(') >= 0 && NEW.indexOf('function aheadLine(') >= 0
  && NEW.indexOf('function aheadByDay(') >= 0 && NEW.indexOf('ไม่พอถึง ') >= 0);
ok('⑥c ของที่กำลังผลิตถูกนับเป็นของที่จะมี (ไม่เตือนผิดว่าไม่พอ)',
  NEW.indexOf('const coming=') >= 0);
ok('⑥d เมนูที่มีใบจ่อวันข้างหน้าต้องไม่หายจากหน้าวางแผน',
  NEW.indexOf('||aheadByDay(r.m.id).length') >= 0);
