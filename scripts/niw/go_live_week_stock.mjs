...
async function openStep(plan,stock){
  ...
  // กวาดเมนูสัปดาห์เก่าที่หมดแล้วออก — นัทสั่ง 14 ก.ย. ("ขึ้นหมดแล้วหมดๆๆๆ แล้วมันน่าเกลียด")
  // ปิดเฉพาะตัวที่ stock_total=0 (ไม่มีอะไรให้ขายแล้ว) · ตัวที่ยังเหลือของ ไม่แตะ ปล่อยขายต่อ
  const oldWk=await g('menu_items?select=code,name,stock_total&is_available=eq.true&is_weekly_special=eq.true&available_from=lt.'+WEEK);
  const sold=oldWk.filter(m=>m.stock_total===0);
  for(const m of sold) if(!await P(m.code,{is_available:false})) console.log('  ❌ ปิดไม่สำเร็จ '+m.code);
  const left=oldWk.filter(m=>m.stock_total!==0);
  console.log('กวาดสัปดาห์เก่า: ปิดที่หมดแล้ว '+sold.length+' ตัว'+(left.length?' · ยังเหลือของอีก '+left.length+' ตัว ('+left.map(m=>m.code+':'+m.stock_total).join(' ')+') ปล่อยขายต่อ':''));
