#!/usr/bin/env node
/* ⛔ ปิดถาวร — นัทเคาะเอง 17 ส.ค. 2569: "ฮาโตะเราไม่ทำอะไรกับมันแล้วนะ"
 *
 * ทำไมต้องปิดแข็งขนาดนี้ (ไม่ใช่แค่เตือน):
 *   17 ส.ค. เมนูพิเศษสัปดาห์ 17-23 ถูกเปิด/ปิดสลับกัน 6 รอบใน 1 เช้า
 *   นัทเสียเวลาทั้งเช้าไล่รีเฟรชเช็ค · สคริปต์กลุ่มนี้เขียน `menu_items` ตรงผ่าน REST
 *   **ไม่ขึ้น `activity_log`** → ตามตัวไม่เจอตั้งนาน กว่าจะรู้ว่าเป็นตัวการ
 *
 * กลไกที่ทำให้พัง: ยึด Hato เป็นแหล่งความจริง
 *   Hato ไม่มี/ขึ้นว่าหมด → ปิดของเรา   ← เมนูสัปดาห์ใหม่โดนยกชุด
 *   Hato ยังขายอยู่      → เปิดของเรา   ← ของที่เราเพิ่งปิดโดนเปิดคืน
 *
 * 🔑 เหตุผลเชิงหลักการ: **เราเลิกใช้ Hato แล้ว Hato ไม่ใช่แหล่งความจริงอีกต่อไป**
 *   เมนูของเราคุมเองที่หน้า DB · เอา Hato มาทับ = ย้อนระบบกลับไปหาของที่ทิ้งไปแล้ว
 *
 * ⚠️ ไม่มีสวิตช์ปลดล็อกด้วย env โดยตั้งใจ — สวิตช์ที่ปลดง่ายจะถูกปลดโดยไม่ตั้งใจ
 *   ถ้าต้องดึงเมนูจาก Hato มาดูชั่วคราวจริงๆ ให้เขียนสคริปต์ใหม่ที่ **อ่านอย่างเดียว**
 *   (ห้าม UPDATE/INSERT ลง menu_items) แล้วประกาศบอร์ดก่อนรัน */
console.error("
⛔ สคริปต์นี้ถูกปิดถาวรแล้ว (17 ส.ค. 2569) — เราเลิกใช้ Hato เป็นแหล่งความจริงแล้ว");
console.error("   เคยทำเมนูสัปดาห์เปิด/ปิดสลับกัน 6 รอบในเช้าเดียว เพราะเอาข้อมูล Hato มาทับของเรา");
console.error("   ถ้าต้องดึงเมนูมาดู ให้เขียนสคริปต์อ่านอย่างเดียว แล้วประกาศบอร์ดก่อนรัน
");
process.exit(1);
